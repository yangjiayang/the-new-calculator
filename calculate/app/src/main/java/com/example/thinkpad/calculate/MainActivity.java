package com.example.thinkpad.calculate;

import java.text.DecimalFormat;
import java.util.StringTokenizer;

import com.example.thinkpad.calculate.R;
...
public class MainActivity extends Activity {
    ...
    //判断是否是按=之前的输入，true表示输入在=之前，false反之
    ...
    public void onCreate(Bundle icicle)
    {
       ...
       inputnext= (TextView)findViewById(R.id.inputnext);
    ...
    private OnClickListener actionPerformed = new OnClickListener() {
        public void onClick(View v) {
            //按键上的命令获取，将view控件转换成button控件执行btn所拥有属性
            String command = ((Button)v).getText().toString();
            //显示器上的字符串
            String str = input.getText().toString();
            //检测输入是否合法，indexof返回字符串首次出现的位置。
            if(equals_flag == false && "0123456789.()sincostanlnlogn!+-×÷√^".indexOf(command) != -1) {
                //检测显示器上的字符串是否合法
                if(right(str)) {
                    if("+-×÷√^)".indexOf(command) != -1) {
                        for(int i =0 ; i < str.length(); i++) {
                            Tipcommand[tip_i] = String.valueOf(str.charAt(i));//charAt是返回指定索引值，String.valueof是将里面的字符型转换成字符串型
                            tip_i++;
                        }
                        vbegin = false;
                    }
            ...
            if("0123456789.()sincostanlnlogn!+-×÷√^".indexOf(command) != -1
                ...
                //一次删除3个字符,如果多于3个字符时候就直接删除三个，如果是只等于三个字符时候就直接设为0了。
                if(TTO(str) == 3) {
                    if(str.length() > 3)
                        input.setText(str.substring(0, str.length() - 3));//截取索引从0到str.length()-3之间的字符
     ...
     */
    /*
    若字符串里面的每一个字符是不等于给定的就跳出循环，意思就是等于给定的，i就+1，如果跟整个
    字符串长度相等说明每一个字符都判定到了，就是对的。
    */
    ...
    public class calc {
        ...
        public void process(String str) {
            ...
            //stringtokenizer是分界符，（需要分析的字符串，后面是分界符，遇到一个就分出来里面的东西。）
            ...
            while (i < expression.length()) {
                ...
                if (i == 0) {
                    if (ch == '-')
                        flag = -1;
                } else if(expression.charAt(i-1) == '(' && ch == '-')//（- 也是-的
                    flag = -1;
                //取得数字，并将正负符号转移给数字
                if (ch <= '9' && ch >= '0'|| ch == '.' || ch == 'E') {
                    ...
                    if (num.compareTo(".") == 0) number[topNum++] = 0;//将点赋值为0
                        //将正负符号转移给数字
                    else {
                ...
                if (ch == '-' && flag == 1 || ch == '+' || ch == '×'|| ch == '÷' ||
                    ...
                    if (topOp == 0 || weight[topOp-1] < weightTemp) {
                        ...
                        operator[topOp] = ch;
                        topOp++;
                        //否则将堆栈中运算符逐个取出，让其与操作数进行计算，然后存入数字栈。
                        // 直到当前堆栈顶部运算符的优先级小于当前运算符
                    }else {
            ...
            //输出最终结果
           inputnext.setText(String.valueOf(FP(number[0])));
